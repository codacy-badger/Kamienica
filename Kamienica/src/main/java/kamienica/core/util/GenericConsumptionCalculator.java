...
//		// usageValue.setDaysBetweenReadings(getDaysBetween(oldReadings,
//		// newReadings));
//		// out.add(usageValue);
//		// }
//
...
//			return 0;
//		} else {
//			return Days.daysBetween(oldReadings.get(0).getReadingDate(), newReadings.get(0).getReadingDate()).getDays();
//		}
//	}
//
...
//
//	@Override
//	public List<UsageValue> calculateConsumption(List<Apartment> apartment, Stream<Reading> readings)
//			throws NegativeConsumptionValue {
//		LocalDate maxDate = readings.map(r -> r.getReadingDate()).max(LocalDate::compareTo).get();
//		LocalDate minDate = readings.map(r -> r.getReadingDate()).min(LocalDate::compareTo).get();
//
//		List<UsageValue> list = new ArrayList<>();
//
//		for (Apartment ap : apartment) {
//			UsageValue usageValue = createNewUsageValue(ap);
//			double totalUsage = 0;
//
//			totalUsage += readings.filter(x -> x.getReadingDate().equals(maxDate)).filter(x -> x.getMeter().getApartment().equals(ap)).mapToDouble(x -> x.getValue()).sum();
//			totalUsage -= readings.filter(x -> x.getReadingDate().equals(minDate)).filter(x -> x.getMeter().getApartment().equals(ap)).mapToDouble(x -> x.getValue()).sum();
//
//			usageValue.setUsage(totalUsage);
//		}
//
