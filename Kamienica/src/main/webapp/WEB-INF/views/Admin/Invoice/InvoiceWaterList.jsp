...
<body>
		<div id="mainWrapper">
		<h1>Lista Faktur Wody</h1>
		<hr>
		<a href="../home.html">Strona Główna</a> <br>
		<a href="registerInvoice.html">Dodaj
			fakturę</a><br> <a href="invoiceList.html">Sprawdź
			pozostałe faktury</a>
			...
			<table width="100%">
				...
				<c:forEach items="${invoice}" var="invoice">
					<tr>
						...
						<td><a
							href="<c:url value="/Admin/Invoice/invoiceWaterDelete.html?id=${invoice.id}"  />"
							onclick="return confirm('Na pewno usunąć?')">Usuń</a></td>
						<td><a
							href="<c:url value="/Admin/Invoice/invoiceWaterEdit.html?id=${invoice.id}" />">Edytuj</a></td>
					</tr>
				</c:forEach>
			</table>
		</c:if>
